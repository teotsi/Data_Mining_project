...
for column in predict_columns:
    ...
    df_test = read_data('test.csv', one_hot=False, extra_csv=column+'.csv')  # random forest is used without one-hot encoding
    ...
    scores = get_scores(column + " AVG prediction", y_test, mean_pred[column]['avg'].tolist())
    with open('log.txt', append_write) as file:
        file.write(scores)
    print(scores)
